...
public interface OrderRepository extends CrudRepository<Order, Long> {
    @Query('select r from Order r where r.user = ?2')
    List<Order> findMyOrder(long userId)

    @Query('select r from Order r where r.community = ?1 and r.user = ?2')
    List<Order> findMyOrder(long communityId, long userId)

    @Query('select r from Order r where r.community = ?1 and r.user = ?2 and status = OPEN')
    List<Order> findMyOpenOrder(long communityId, long userId)

    @Query('select o from Order o where o.user = ?1 and status = ?2')
